...
export const useUserStore = defineStore(
  ...
  () => {
    ...
    const setUserInfo = (newInfo: Api.Auth.UserInfo) => {
      info.value = newInfo
      permissions.value = newInfo.permissions || newInfo.buttons || []
    }
     ...
     * 判断用户是否拥有权限标识
     */
    const hasPermission = (permission: string | string[], mode: 'some' | 'every' = 'some') => {
      ...
      return mode === 'every'
        ? requiredPermissions.every((item) => permissionList.includes(item))
        : requiredPermissions.some((item) => permissionList.includes(item))
    }
