...
function checkAuthPermission(el: HTMLElement, binding: DirectiveBinding<string | string[]>): void {
  ...
  const requiredAuth = Array.isArray(binding.value) ? binding.value : [binding.value]

  if (userStore.hasPermission(requiredAuth)) {
    return
  }
  ...
  // 检查是否有对应的权限标识
  const hasPermission = authList.some((item) => requiredAuth.includes(item.authMark))
